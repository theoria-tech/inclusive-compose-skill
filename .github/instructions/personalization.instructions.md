---
description: パーソナライズと文字サイズ。OSのフォントスケール・表示設定を尊重する。テキスト表示/サイズ/レイアウト制約を扱うときに適用。
applyTo: '**/*.kt,**/*.kts'
---
...
- 本文の `lineHeight` はフォントサイズの約1.5倍（WCAG 1.4.8）。字間 `letterSpacing` を触るなら詰めすぎない（WCAG 1.4.12 Text Spacing）。Material タイポグラフィ（`bodyLarge` 等）は概ね満たす。
